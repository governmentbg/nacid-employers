      ...
      this.resource.changePassword(this.model)
        .subscribe(() => {
          ...
          (err) => handleDomainError(
            err,
            [
              { code: 'User_ChangePasswordOldPasswordMismatch', text: this.translateService.instant('toastr.userInvalidOldPassword') },
              { code: 'User_ChangePasswordNewPasswordMismatch', text: this.translateService.instant('toastr.userChangePasswordNewPasswordMismatch') }
            ],
            this.toastrService
          )
        );
